...
let btn_edit = window.document.getElementById('btn_edit');
let btn_cancel = window.document.getElementById('btn_cancel');
let edit_tank = window.document.getElementById('edit_tank');
let edit_config = window.document.getElementById('edit_config');
...
this.btn_edit.addEventListener('click', edit);
this.btn_cancel.addEventListener('click', cancel);
...
function increment() {
  ...
  val_levelWater.style.height = (parseInt(lbl_lts.innerHTML) * 100) / maxHighLevel + '%';
  systemDetect();
}
...
function decrement() {
  ...
  lbl_lts.innerHTML = parseInt(lbl_lts.innerHTML) - 1;
  if (lbl_lts.innerHTML == 0) {
    val_levelWater.style.height  = '0%';
  } else {
    val_levelWater.style.height = (parseInt(lbl_lts.innerHTML) * 100 ) / maxHighLevel + '%';
  }
...
function setLts() {
  ...
  if (parseInt(fld_point.value) < maxLowLevel || parseInt(fld_point.value) > maxHighLevel) {
    alert("El valor en litros debe estar entre: " + maxLowLevel + " y " + maxHighLevel);
...
function configControls() {
  if (initial) {
    ...
    if (parseInt(lbl_lts.innerHTML) >= maxLowLevel && parseInt(lbl_lts.innerHTML) <= maxHighLevel) {
      val_levelWater.style.height = (parseInt(lbl_lts.innerHTML) * 100 ) / maxHighLevel + '%';
    } else{
      lbl_lts.innerHTML = maxLowLevel;
      val_levelWater.style.height = (maxLowLevel * 100) / maxHighLevel + '%';
    }
    if (alarmHighLevelConfig > maxHighLevel) {
      alarmHighLevelConfig = maxHighLevel;
      fld_high_level.value = alarmHighLevelConfig;
    }
    if (alarmLowLevelConfig < maxLowLevel) {
      alarmLowLevelConfig = maxLowLevel;
      fld_low_level.value = alarmLowLevelConfig;
    }
    if (alarmLowLevelConfig >= maxHighLevel) {
      alarmLowLevelConfig = maxLowLevel;
      fld_low_level.value = alarmLowLevelConfig;
    }
  }
...
function edit() {
  edit_tank.style.display = 'initial';
  edit_config.style.display = 'none';
}

function cancel() {
  edit_tank.style.display = 'none';
  edit_config.style.display = 'initial';
}
